...
impl BreakoutCalculator {
    ...
    pub fn calculate_breakout_signal(
        ...
        let lookback_prices: Vec<f64> = history.prices
            .iter()
            .rev()
        ...
        let lookback_high = lookback_prices.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
        ...
        let (breakout_type, breakout_level) = self.determine_breakout_type(
            current_price,
            lookback_high,
            lookback_low,
            volatility,
    ...
    pub fn breakout_to_signal_strength(&self, breakout: &BreakoutSignal) -> f64 {
        match breakout.breakout_type {
            ...
            BreakoutType::UpBreakout => {
                ...
                let magnitude = (breakout.current_price - breakout.breakout_level) / breakout.breakout_level;
            ...
            BreakoutType::DownBreakout => {
                ...
                let magnitude = (breakout.breakout_level - breakout.current_price) / breakout.breakout_level;
                let base_strength = magnitude * 100.0; // Convert to percentage
