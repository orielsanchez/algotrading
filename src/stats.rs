...
pub fn sharpe_ratio(returns: &[f64], risk_free_rate: f64, periods_per_year: f64) -> Result<f64> {
    ...
    if std_dev == 0.0 {
        return Err(anyhow!("Cannot calculate Sharpe ratio: zero standard deviation"));
    }
...
pub fn information_ratio(returns: &[f64], benchmark_returns: &[f64]) -> Result<f64> {
    ...
    if returns.is_empty() {
        return Err(anyhow!("Cannot calculate information ratio for empty returns"));
    }
...
impl PortfolioStats {
    pub fn calculate(
        ...
        if returns.is_empty() {
            return Err(anyhow!("Cannot calculate portfolio stats for empty returns"));
        }
