...
impl RiskBudgeter {
    ...
    pub fn update_correlation(
        ...
        self.correlation_matrix
            .insert((symbol1.to_string(), symbol2.to_string()), correlation);
        self.correlation_matrix
            .insert((symbol2.to_string(), symbol1.to_string()), correlation);
    ...
    pub fn update_volatility(&mut self, symbol: &str, volatility: f64) -> Result<()> {
        if volatility < 0.0 {
            return Err(anyhow::anyhow!(
                "Volatility cannot be negative, got {}",
                volatility
            ));
        }
    ...
    pub fn calculate_risk_contributions(&self, portfolio: &Portfolio) -> Result<RiskAttribution> {
        ...
        let weighted_avg_volatility: f64 = positions
            .iter()
            .map(|(symbol, _)| {
                let weight = weights.get(symbol).copied().unwrap_or(0.0);
                let volatility = self.volatilities.get(symbol).copied().unwrap_or(0.0);
        ...
        let concentration_score = if risk_contributions.is_empty() {
            ...
            let hhi: f64 = risk_contributions
                .iter()
    ...
    pub fn calculate_erc_allocations(&self, portfolio: &Portfolio) -> Result<Vec<ERCAllocation>> {
        ...
        for risk_contrib in &current_attribution.risk_contributions {
            ...
            let target_weight = if risk_contrib.marginal_risk > 0.0 {
                (target_risk_contribution * current_attribution.total_portfolio_volatility)
        ...
        let total_target_weight: f64 = erc_allocations
            .iter()
            .map(|alloc| alloc.target_weight)
            .sum();
                        ...
                        let already_clustered =
                            correlation_clusters
                                ...
                                .any(|existing_cluster: &Vec<String>| {
                                    existing_cluster.contains(symbol_j)
                                });
                ...
                let overlaps_existing =
                    correlation_clusters
                        ...
                        .any(|existing_cluster: &Vec<String>| {
                            cluster
                                .iter()
                                .any(|symbol| existing_cluster.contains(symbol))
                        });
    ...
    pub fn check_risk_budget_violations(&self, risk_attribution: &RiskAttribution) -> Vec<String> {
        ...
        for contribution in &risk_attribution.risk_contributions {
            ...
            if relative_deviation > violation_threshold
                || excessive_risk_budget
                || excessive_risk_contribution
            {
    ...
    pub fn generate_rebalancing_recommendations(
        ...
        recommendations.sort_by(|a, b| {
            b.adjustment_needed
                .abs()
                .partial_cmp(&a.adjustment_needed.abs())
...
mod tests {
    ...
    fn create_test_portfolio() -> Portfolio {
        ...
        portfolio.register_security(
            "AAPL".to_string(),
            SecurityInfo::new_stock("AAPL".to_string(), "SMART".to_string(), "USD".to_string()),
        );
        portfolio.register_security(
            "SPY".to_string(),
            SecurityInfo::new_stock("SPY".to_string(), "SMART".to_string(), "USD".to_string()),
        );
        portfolio.register_security(
            "QQQ".to_string(),
            SecurityInfo::new_stock("QQQ".to_string(), "SMART".to_string(), "USD".to_string()),
        );
    ...
    fn test_correlation_update_invalid_range() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Correlation must be between -1 and 1")
        );
    ...
    fn test_volatility_update_negative() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("Volatility cannot be negative")
        );
    }
        ...
        assert!(
            result.is_err()
                || {
                    ...
                    let total_risk: f64 = attribution
                        .risk_contributions
                        .iter()
                        .map(|rc| rc.risk_contribution)
                        .sum();
