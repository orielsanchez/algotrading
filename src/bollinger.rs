...
impl BollingerCalculator {
    ...
    pub fn calculate_bollinger_signal(
        ...
        let signal_strength = self.calculate_raw_signal_strength(&bands, current_price, &signal_type);
    ...
    fn determine_signal_type(&self, bands: &BollingerBands, current_price: f64) -> BollingerSignalType {
    ...
    fn calculate_raw_signal_strength(&self, bands: &BollingerBands, current_price: f64, signal_type: &BollingerSignalType) -> f64 {
        match signal_type {
            ...
            BollingerSignalType::Breakout => {
                ...
                if current_price > bands.upper_band {
                    ...
                    let breakout_strength = (current_price - bands.upper_band) / bands.std_deviation;
                    breakout_strength.min(1.0)
                } else if current_price < bands.lower_band {
                    // Downward breakout - negative signal
                    let breakout_strength = (bands.lower_band - current_price) / bands.std_deviation;
                    -breakout_strength.min(1.0)
                } else {
                    0.0
                }
    ...
    pub fn calculate_multi_timeframe_bollinger(
        ...
        let dominant_signal = timeframe_signals
            .values()
            .max_by(|a, b| a.signal_strength.abs().partial_cmp(&b.signal_strength.abs()).unwrap())
            .cloned();
        
        // Determine volatility regime based on band squeeze conditions
        let squeeze_count = timeframe_signals.values().filter(|s| s.band_squeeze).count();
        ...
        let volatility_regime = if squeeze_count > total_signals / 2 {
            ...
            let avg_bandwidth: f64 = timeframe_signals.values()
...
mod tests {
    ...
    fn test_basic_bollinger_bands_calculation() {
        ...
        let bands = calc.calculate_bollinger_bands(&prices).expect("Should calculate bands");
