...
impl VolatilityCalculator {
    ...
    pub fn new(half_life: f64, annualization_factor: f64) -> Self {
        ...
        info!(
            "Initializing volatility calculator with half-life: {:.1} days, lambda: {:.4}",
            half_life, lambda
        );
    ...
    fn update_volatility_estimate(&mut self, symbol: &str, new_return: f64) {
        ...
        if returns_len == 1 {
            ...
            self.volatility_estimates
                .insert(symbol.to_string(), initial_vol);

            debug!(
                "Initial volatility estimate for {}: {:.4}",
                symbol, initial_vol
            );
        } else {
            ...
            let new_variance =
                self.lambda * old_variance + (1.0 - self.lambda) * new_return * new_return;
            let new_vol = (new_variance * self.annualization_factor).sqrt();

            self.volatility_estimates
                .insert(symbol.to_string(), new_vol);

            debug!(
                "Updated volatility for {}: {:.4} (from {:.4})",
                symbol, new_vol, current_vol
            );
        }
    }
...
impl VolatilityTargeter {
    ...
    pub fn new(target_volatility: f64, risk_config: RiskConfig) -> Self {
        ...
        info!(
            "Initializing volatility targeter with target: {:.1}%",
    ...
    pub fn calculate_position_size(
        ...
        let position_size =
            (signal_strength * self.target_volatility * portfolio_value) / (instrument_vol * price);
