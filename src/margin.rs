...
pub fn get_futures_margin_requirements(symbol: &str) -> MarginRequirements {
    match symbol {
        ...
        _ => MarginRequirements {         // Conservative default
...
pub fn calculate_initial_margin(
    ...
    match security_info.security_type {
        ...
        SecurityType::Future => {
            let contract = security_info.contract_specs.as_ref()
            ...
            info!("Initial margin for {} contracts of {}: ${:.2} ({}% of ${:.2} value)",
                quantity, security_info.symbol, margin, 
                margin_req.initial_percentage * 100.0, contract_value);
...
pub fn validate_margin_requirements(
    ...
    // Get account values
    let available_funds = account_summary.get("AvailableFunds")
        .copied()
        .unwrap_or(0.0);
    let initial_margin_req = account_summary.get("InitMarginReq")
        .copied()
        .unwrap_or(0.0);
    let _maintenance_margin_req = account_summary.get("MaintMarginReq")
        .copied()
        .unwrap_or(0.0);
    let net_liquidation = account_summary.get("NetLiquidation")
        .copied()
        .unwrap_or(0.0);
    
    // Calculate margin for the new position
    let new_position_margin = calculate_initial_margin(
        &order.security_info,
        order.quantity,
        order.price
    )?;
    ...
    if margin_utilization_after > max_margin_utilization {
        ...
        warning_message = Some(format!(
            ...
            margin_utilization_after * 100.0, max_margin_utilization * 100.0
        ));
    }
...
pub fn check_margin_health(
    ...
    let maintenance_margin = account_summary.get("MaintMarginReq")
        .copied()
        .unwrap_or(0.0);
    let net_liquidation = account_summary.get("NetLiquidation")
        .copied()
        .unwrap_or(0.0);
    let excess_liquidity = account_summary.get("ExcessLiquidity")
        .copied()
        .unwrap_or_else(|| {
    ...
    if net_liquidation == 0.0 {
        return MarginStatus::Warning("Unable to determine margin status: zero net liquidation".to_string());
    }
    ...
    if margin_utilization > margin_call_threshold {
        return MarginStatus::Critical(format!(
            ...
            margin_utilization * 100.0, margin_call_threshold * 100.0, excess_liquidity
        ));
    }
    ...
    if margin_utilization > margin_call_threshold * 0.8 {
        return MarginStatus::Warning(format!(
            ...
            margin_utilization * 100.0, excess_liquidity
        ));
    }
    
    // Warning: Low cushion
    if margin_cushion < 0.15 { // Less than 15% cushion
        return MarginStatus::Warning(format!(
    ...
    for (symbol, position) in positions {
        if let Some(&current_price) = market_data.get(symbol) {
            if let Some(ref security_info) = position.security_info {
                let initial = calculate_initial_margin(
                    security_info,
                    position.quantity,
                    current_price
                )?;
                let maintenance = calculate_maintenance_margin(
                    security_info,
                    position.quantity,
                    current_price
                )?;
