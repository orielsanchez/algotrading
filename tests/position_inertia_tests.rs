...
use algotrading::position_inertia::{InertiaConfig, InertiaDecision, PositionInertiaCalculator};
...
mod position_inertia_tests {
    ...
    fn test_small_position_change_blocked_by_inertia() -> Result<()> {
        ...
        let target_position = 1150.0; // $1150 target position ($150 change)
        let transaction_cost = 80.0; // $80 transaction cost
        ...
        // Position change ($150) > minimum ($100) but < inertia threshold ($160), so should hold
        assert_eq!(
            decision.action,
        ...
        assert_eq!(
            decision.recommended_position, current_position,
            "Should recommend holding current position"
        );
        assert!(
            decision.reason.contains("inertia"),
    ...
    fn test_large_position_change_overrides_inertia() -> Result<()> {
        ...
        assert_eq!(
            decision.recommended_position, target_position,
            "Should recommend target position"
        );
        assert!(
            decision.reason.contains("threshold exceeded"),
    ...
    fn test_strong_signal_overrides_inertia() -> Result<()> {
        ...
        let current_position = 1000.0; // $1000 current position
        let target_position = 1130.0; // $1130 target position ($130 change)
        let transaction_cost = 70.0; // $70 transaction cost
        ...
        // Position change ($130) < inertia threshold ($140), but strong signal should override
        assert_eq!(
            decision.action,
            InertiaDecision::Rebalance,
        ...
        assert!(
            decision.reason.contains("Strong signal"),
            "Decision reason should mention strong signal. Actual reason: '{}'", decision.reason
        );
    ...
    fn test_negative_signal_triggers_position_reduction() -> Result<()> {
        ...
        let transaction_cost = 50.0; // $50 transaction cost
        let signal_strength = -18.0; // Very strong negative signal
        let price = 100.0;
        ...
        // Strong negative signal should reduce position, but may be limited by max change
        let expected_position = current_position * (1.0 - 0.50); // 50% max reduction = $500
        assert_eq!(
            decision.recommended_position, expected_position,
            "Should reduce position but limited by maximum change percentage"
        );
    ...
    fn test_position_reversal_always_executes() -> Result<()> {
        ...
        assert!(
            decision.reason.contains("reversal"),
    ...
    fn test_minimum_position_change_threshold() -> Result<()> {
        ...
        assert!(
            decision.reason.contains("minimum change"),
    ...
    fn test_maximum_position_change_limiting() -> Result<()> {
        ...
        assert!(
            decision.reason.contains("limited"),
