...
mod trading_integration_tests {
    ...
    fn create_test_signal(symbol: &str, quantity: f64, price: f64, _signal_strength: f64) -> OrderSignal {
        OrderSignal {
            ...
            security_info: SecurityInfo::new_stock(symbol.to_string(), "SMART".to_string(), "USD".to_string()),
        }
    }
    ...
    async fn test_signal_filtering_with_inertia_enabled() -> Result<()> {
        ...
        let signals = vec![
            ...
            create_test_signal("AAPL", 102.0, 155.0, 5.0), // $310 change vs $60 threshold
        ...
        let (filtered_signals, filter_result) = integration_layer
            .filter_signals_with_cost_optimization(
                ...
                &portfolio,
                &latest_prices,
                15.0, // 15 bps max cost
            )
            .await?;

        // Should filter out small AAPL change due to inertia
        assert_eq!(filter_result.original_signals, 3);
        assert_eq!(filter_result.inertia_filtered, 1); // AAPL blocked
        assert!(filter_result.final_signals >= 2); // GOOGL and MSFT should pass

        // Verify AAPL was filtered out
        assert!(!filtered_signals.iter().any(|s| s.symbol == "AAPL"));
        
        // Verify GOOGL and MSFT passed
    ...
    async fn test_order_cost_validation() -> Result<()> {
        ...
        let is_valid = integration_layer
            .validate_order_cost(&normal_signal, 155.0, 15.0)
            .await?;
        ...
        let is_valid_high_cost = integration_layer
            .validate_order_cost(&high_cost_signal, 0.01, 15.0)
            .await?;
    ...
    async fn test_transaction_cost_estimation() -> Result<()> {
        ...
        assert!(estimated_cost > 1.0, "Should include at least $1 commission");
    ...
    async fn test_filter_result_metrics() -> Result<()> {
        ...
        assert_eq!(filter_result.inertia_filtered, 1); // AAPL
        assert_eq!(filter_result.cost_filtered, 1);    // PENNY
        assert_eq!(filter_result.final_signals, 2);    // GOOGL, MSFT
