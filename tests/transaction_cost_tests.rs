...
use algotrading::transaction_cost::{
    MarketImpactModel, TransactionCostCalculator, TransactionCostConfig,
};
use anyhow::Result;
...
mod transaction_cost_tests {
    ...
    fn test_bid_ask_spread_cost_calculation() -> Result<()> {
        ...
        let stock_cost =
            calculator.calculate_spread_cost("AAPL", &SecurityType::Stock, 100.0, 150.0)?;
        let expected_stock_cost = 150.0 * 100.0 * 0.0005; // price * quantity * spread
        assert!(
            (stock_cost - expected_stock_cost).abs() < 0.01,
            "Stock spread cost: expected {}, got {}",
            expected_stock_cost,
            stock_cost
        );
        ...
        let forex_cost =
            calculator.calculate_spread_cost("EURUSD", &SecurityType::Forex, 10000.0, 1.0850)?;
        let expected_forex_cost = 1.0850 * 10000.0 * 0.0020;
        assert!(
            (forex_cost - expected_forex_cost).abs() < 0.01,
            "Forex spread cost: expected {}, got {}",
            expected_forex_cost,
            forex_cost
        );
        ...
        let futures_cost =
            calculator.calculate_spread_cost("ES", &SecurityType::Future, 1.0, 4200.0)?;
        let expected_futures_cost = 4200.0 * 1.0 * 0.0010;
        assert!(
            (futures_cost - expected_futures_cost).abs() < 0.01,
            "Futures spread cost: expected {}, got {}",
            expected_futures_cost,
            futures_cost
        );
    ...
    fn test_commission_cost_calculation() -> Result<()> {
        ...
        let forex_commission =
            calculator.calculate_commission_cost(&SecurityType::Forex, 10000.0)?;
        assert_eq!(forex_commission, 0.50, "Forex commission should be $0.50");

        // Test futures commission (per contract)
        let futures_commission =
            calculator.calculate_commission_cost(&SecurityType::Future, 5.0)?;
        assert_eq!(
            futures_commission, 12.50,
            "5 futures contracts should cost $12.50"
        );
    ...
    fn test_market_impact_scaling() -> Result<()> {
        ...
        let small_position_cost = calculator.calculate_market_impact_cost(
            "AAPL", 66.0, 150.0,
            1000000.0, // 66 shares * $150 = $9,900 = 0.99% of daily volume
        )?;
        assert_eq!(
            small_position_cost, 0.0,
            "Small positions should have no market impact"
        );

        // Test large position (with market impact)
        let large_position_cost = calculator.calculate_market_impact_cost(
            "AAPL", 100.0, 150.0,
            1000000.0, // 100 shares * $150 = $15,000 = 1.5% of daily volume
        )?;
        ...
        assert!(
            (large_position_cost - expected_impact).abs() < 0.01,
            "Large position impact: expected {}, got {}",
            expected_impact,
            large_position_cost
        );
    ...
    fn test_total_transaction_cost_calculation() -> Result<()> {
        ...
        let total_cost = calculator.calculate_total_cost(
            ...
            66.0,
            150.0,
            1000000.0, // 66 shares = 0.99% of daily volume
        )?;
        ...
        assert!(
            (total_cost - expected_total).abs() < 0.01,
            "Total transaction cost: expected {}, got {}",
            expected_total,
            total_cost
        );
    ...
    fn test_round_trip_cost_calculation() -> Result<()> {
        ...
        assert!(
            (round_trip_cost - expected_round_trip).abs() < 0.01,
    ...
    fn test_unknown_symbol_fallback() -> Result<()> {
        ...
        let unknown_cost =
            calculator.calculate_spread_cost("UNKNOWN", &SecurityType::Stock, 100.0, 50.0)?;
        let expected_fallback = 50.0 * 100.0 * 0.0010; // Default 0.10% spread
        assert!(
            (unknown_cost - expected_fallback).abs() < 0.01,
    ...
    fn test_zero_quantity_handling() -> Result<()> {
        ...
        let zero_cost =
            calculator.calculate_total_cost("AAPL", &SecurityType::Stock, 0.0, 150.0, 1000000.0)?;
    ...
    fn test_negative_quantity_handling() -> Result<()> {
        ...
        assert!(
            (negative_cost - positive_cost).abs() < 0.01,
